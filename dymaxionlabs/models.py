...
class Model:
    """
    The Model class represents a machine learning model that can be trained to solve
    different kinds of tasks, like object detection or classification.

    To instance a Model, you should use one of the following class
    methods: :meth:`all()`, :meth:`get()`, or :meth:`create()`.

    :param str uuid: internal id
    :param str name: name
    :param list classes: list of labels/classes
    :param str model_type: type of model (i.e. )
    :param list image_files: list of associated image files used for training
    :param dict metadata: user metadata
    :param dict configuration: model configuration
    :param dict extra_attributes: extra attributes from API endpoint
    ...
    @classmethod
    def all(cls):
        """Fetches all models in the current project."""
        return [
            cls._from_attributes(**attrs)
            for attrs in fetch_from_list_request(f'{cls.base_path}/')
        ]
    ...
    @classmethod
    def get(cls, uuid):
        """Gets a model identified by ``uuid``.

        :param uuid str: Model UUID
        :rtype: Model
    ...
    def create(cls, *, name, type, classes=[], metadata=None,
               configuration={}):
        """Creates a new Model named ``name`` of ``type`` with
        ``classes`` as labels with ``training_hours`` hour of training job.
        ...
        :param name str: A name to identify the model
        :param type str: Type of model ("object_detection")
        :param classes list: Optional list of classes/labels of objects
        :param metadata dict: Optional metadata dictionary to store extra attributes
        :param configuration dict: Optional configuration dictionary for training
        :rtype: Model
    ...
    def delete(self):
        ...
        :returns: ``True`` if model was succesfully deleted.
    ...
    def add_image(self, *images):
        """Adds an image :class:`File` to the model, for training.
    ...
    def train(self):
        """Start a training job using this model.

        It will build a custom model based on all the images from the
        model and the associates annotations.
    ...
    def __repr__(self):
        return "<Model uuid={uuid!r} name={name!r}>".format(name=self.name,
                                                                uuid=self.uuid)
